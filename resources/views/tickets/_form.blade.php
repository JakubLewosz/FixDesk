@if($categories->isEmpty())
    <div class="notice error">Brak kategorii. Wczytaj dane początkowe poleceniem php artisan db:seed.</div>
@endif

@foreach([
    'title' => ['Tytuł', 5, 120],
    'location' => ['Lokalizacja', 2, 100],
    ...
    <div class="field">
        ...
        <input
            id="{{ $field }}"
            name="{{ $field }}"
            value="{{ is_string(old($field, $ticket->$field)) ? old($field, $ticket->$field) : '' }}"
            required
            minlength="{{ $min }}"
            maxlength="{{ $max }}"
        ...
        @error($field)
            <p class="field-error" id="{{ $field }}-error">{{ $message }}</p>
        @enderror
    </div>
@endforeach
