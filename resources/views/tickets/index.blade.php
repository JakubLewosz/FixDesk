...
@foreach(App\Enums\TicketStatus::cases() as $status)<option value="{{ $status->value }}" @selected(($filters['status'] ?? '') === $status->value)>{{ $status->label() }}</option>
@endforeach</select>
</div>
<div>
...
@foreach($categories as $category)<option value="{{ $category->id }}" @selected(($filters['category_id'] ?? '') == $category->id)>{{ $category->name }}</option>
@endforeach</select>
</div>
...
@if($tickets->isEmpty())<div class="panel empty">
<h2>{{ ($filters['status'] ?? null) || ($filters['category_id'] ?? null) ? 'Brak wyników filtrowania' : ($view === 'archived' ? 'Archiwum jest puste' : 'Brak zgłoszeń') }}</h2>
<p>Zmień filtry lub wróć do pierwszej strony listy.</p>
<a href="{{ route('tickets.index', ['view' => $view]) }}">Pokaż cały wybrany widok</a>
</div>

@else<div class="panel table-wrap">
<table>
<thead>
<tr>
<th>Numer</th>
